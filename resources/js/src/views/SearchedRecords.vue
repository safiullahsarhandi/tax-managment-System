<template>
    <div>
        <!-- {{ typrOf(searchedData) }} -->
        <div v-for="item,indextr in records()">
        ...
        <br>
        <vs-pagination v-if="objectLength > 0 " @input="records()" v-model="currentPage" :total="totalSearchPages"></vs-pagination>
        <vs-row>
        	<vs-col>
...
<script>
	...
	methods: {
		...
		this.$store.dispatch('get_search_data',{query: val,page : this.currentPage})
		},
		records(){
			let offset = Math.ceil(((this.currentPage - 1) * this.perPage));
			let records = _.filter(this.searchedData,(value,index)=>{ return (index >= offset && index < (offset + this.perPage))});
			return records;
		},
		makeUrl(obj){ 
